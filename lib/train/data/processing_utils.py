...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    if output_sz is not None:
        ...
        im_crop_padded = cv.resize(im_crop_padded, (output_sz, output_sz), interpolation=cv.INTER_CUBIC)     # 改变插值方式
        att_mask = cv.resize(att_mask, (output_sz, output_sz)).astype(np.bool_)
        if mask is None:
            return im_crop_padded, resize_factor, att_mask
        mask_crop_padded = \
        F.interpolate(mask_crop_padded[None, None], (output_sz, output_sz), mode='bicubic', align_corners=False)[0, 0]
