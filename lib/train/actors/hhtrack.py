...
class hypertrackActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        key_to_remove = 'pred_scores'
        if key_to_remove in out_dict:
            del out_dict[key_to_remove]
    ...
    def forward_pass(self, data, run_score_head):
        ...
        if self.settings.script_name == "hypertrack_nir":
            out_dict, _ = self.net(data['template_images'][0],
                                   data['template_images'][1],
                                   data['search_images'],
                                   run_score_head=run_score_head, gt_bboxes=search_bboxes)
        elif self.settings.script_name == "hypertrack_vis":
            out_dict, _ = self.net(data['template_images'][0],
                                   data['template_images'][1],
                                   data['search_images'],
                                   run_score_head=run_score_head, gt_bboxes=search_bboxes)
        elif self.settings.script_name == "hypertrack_rednir":
            out_dict, _ = self.net(data['template_images'][0],
                                   data['template_images'][1],
                                   data['search_images'][:, :, :15, :, :],
                                   run_score_head=run_score_head, gt_bboxes=search_bboxes)
