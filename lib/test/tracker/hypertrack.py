...
import cv2
import os
from lib.models.HyperTrack import build_hypertrack_vit_online_score
from lib.test.tracker.tracker_utils import Preprocessor_wo_mask
from lib.utils.box_ops import clip_box


class hypertrackOnline(BaseTracker):
    def __init__(self, params, dataset_name):
        super(hypertrackOnline, self).__init__(params)

        if dataset_name == "whisper_rednir":
            self.datatype = "hypertrack_rednir"
            self.instance = torch.zeros(1, 15).cuda()
        elif dataset_name == "whisper_vis":
            self.datatype = "hypertrack_vis"
            self.instance = torch.zeros(1, 16).cuda()
        elif dataset_name == "imec25":
            self.datatype = "hypertrack_nir"
            self.instance = torch.zeros(1, 25).cuda()
            dataset_name = "whisper_nir"
        else:
            self.datatype = "hypertrack_nir"
            self.instance = torch.zeros(1, 25).cuda()
        network = build_hypertrack_vit_online_score(params.cfg,  train=False, datatype=self.datatype)
        ...
        if hasattr(self.cfg.TEST.UPDATE_INTERVALS, DATASET_NAME):
            self.update_intervals = self.cfg.TEST.UPDATE_INTERVALS[DATASET_NAME]   # 更新的间隔frame
    ...
    def initialize(self, image, info: dict):
        ...
        if self.online_size > 1:
            with torch.no_grad():
                self.instance = self.network.set_online(self.template, self.online_template)

        # numbers_list = self.instance.squeeze().tolist()
        # print(numbers_list)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            if self.online_size == 1:
                ...
                out_dict, _ = self.network.forward_test(search, self.instance, run_score_head=True)
        ...
        if self.frame_id % self.update_interval == 0:
            ...
            if self.online_size > 1:
                with torch.no_grad():
                    self.instance = self.network.online_update(self.template, self.online_template, self.instance)
