...
import time
import cv2 as cv
from lib.test.utils.HyperTools import *
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, dataset_name: str, run_id: int = None, display_name: str = None,
        ...
        env = env_settings()  # 获得所有数据集、以及results的path
        ...
        tracker_module_abspath = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                                              '..', 'tracker', '%s.py' % "hypertrack"))    # 获得tracker的定义绝对路径
        if os.path.isfile(tracker_module_abspath):
            tracker_module = importlib.import_module('lib.test.tracker.{}'.format("hypertrack"))
    ...
    def _read_image(self, image_file: str):
        if isinstance(image_file, str):
            im = cv.imread(image_file, cv.IMREAD_UNCHANGED)
            if self.name == "hypertrack_nir" or self.name == "imec25":
                im = X2Cube(im, [5, 5], [5, 5], 25)
            else:
                im = X2Cube(im, [4, 4], [4, 4], 16)
            return im
