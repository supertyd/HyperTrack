...
class imec25Dataset(BaseDataset):

    def __init__(self):
        super().__init__()

        self.base_path = self.env_settings.imec25
    ...
    def _construct_sequence(self, sequence_info):
        ...
        return Sequence(sequence_info['name'], frames, 'imec25', ground_truth_rect[init_omit:],
    ...
    def _get_sequence_info_list(self):
        ...
        folder_path_false = self.base_path + "/test"
        # 使用列表推导式获取子目录的文件夹名字
        ...
        for i in name:
            i = i
            count = 0
            for file in os.listdir(folder_path_false + "/" + i + "/imgs"):
        ...
        for i, j in zip(name, number):
            seq = {"name": i, "path": "/test/" + i + "/imgs", "startFrame": 1, "endFrame": j, "nz": 4, "ext": "tif",
                   "anno_path": "/anno-test/"+i + ".txt", "object_class": i}
