...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, help='training script name,hypertrack_nir,hypertrack_vis,hypertrack_rednir', default="hypertrack_vis")
    parser.add_argument('--config', type=str, default='baseline_large', help='yaml configure file name')
    parser.add_argument('--stage1_model', type=str, default="/home/ubuntu/Downloads/hypertrack/models/mixformer_convmae_base_online.pth.tar", help='stage1 model used to train SPM.')
    parser.add_argument('--save_dir', type=str, help='root directory to save checkpoints, logs, and tensorboard', default="/home/ubuntu/Downloads/hypertrack/")
    parser.add_argument('--mode', type=str, choices=["single", "multiple"], default="single",
                        help="train on single gpu or multiple gpus")
    parser.add_argument('--nproc_per_node', type=int, help="number of GPUs per node", default=1)  # specify when mode is multiple
    ...
    parser.add_argument('--config_prv', type=str, default='baseline_large', help='yaml configure file name')
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python /home/ubuntu/Downloads/hypertrack/lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
